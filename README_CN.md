...
>
> 来试试 Ollama-Hack 吧！它是一个基于 Go 的中转平台，能够帮助你轻松管理、测试和无缝使用多个 Ollama 接口。

Ollama-Hack V2 是一个用于管理、测试和转发 Ollama API 的高性能服务。它可以集中管理多个 Ollama 端点，通过 **AI 智能路由**自动将请求分配到最合适的模型，使用**语义缓存**加速相似请求，并通过**零延迟竞速模式**获得最快响应。平台提供 100% 兼容 OpenAI 的 API 和友好的 Web 界面。
...
### 🧠 V2 高级智能功能
-   🎯 **智能路由**：AI 驱动的提示词分类，根据任务类型自动路由到最优模型（编程 → CodeLlama，创意写作 → Mistral，数学 → DeepSeek 等）
-   💾 **语义缓存**：基于相似度的缓存系统，即使提示词不完全相同，也能为语义相似的请求提供即时响应
-   🏥 **健康追踪**：实时监控端点健康状态，根据故障模式自动禁用/重新启用
-   🚀 **零延迟竞速**：同时向所有可用端点发起并行请求，最快响应者获胜！
-   🔪 **Map-Reduce 代理**：自动将大型文档分割到多个 GPU 进行并行处理
-   🔥 **永不休眠模式**：注入无限 `keep_alive` 将模型锁定在 VRAM 中，消除冷启动延迟

## 🛠️ 环境要求

-   Docker 和 Docker Compose（推荐）
-   或 Go 1.22+ 和 Node.js（直接运行）
...
# 启动服务 (请确保你配置了环境变量，例如 DATABASE_HOST)
go run ./cmd/server
```
...
environment:
    # 核心设置
    - APP_ENV=prod # 环境类型：dev 或 prod
    - APP_LOG_LEVEL=INFO # 日志级别
    - APP_SECRET_KEY=change_this_key # JWT密钥
    - APP_ACCESS_TOKEN_EXPIRE_MINUTES=30 # 访问令牌过期时间
    - DATABASE_ENGINE=mysql # 数据库引擎
    - DATABASE_HOST=db # 数据库主机
    - DATABASE_USERNAME=user # 数据库用户名
    - DATABASE_PASSWORD=password # 数据库密码
    - DATABASE_DB=ollama_hack # 数据库名称
    ...
    - SEMANTIC_CACHE_MODEL=nomic-embed-text # 嵌入模型
    - SEMANTIC_CACHE_OLLAMA_URL=http://localhost:11434
    
    # 🏥 健康追踪（自动禁用故障端点）
    - HEALTH_TRACKER_ENABLED=true
    - HEALTH_TRACKER_DISABLE_THRESHOLD=3 # 禁用前的失败次数
    - HEALTH_TRACKER_DISABLE_DURATION=5m # 禁用时长
    - HEALTH_TRACKER_PROBE_INTERVAL=30s # 健康检查间隔
```

### 🎯 智能路由分类

智能路由器将提示词分类并路由到专门的模型：

| 分类 | 关键词 | 默认模型 |
|------|--------|----------|
| 编程 | code, debug, python, javascript, api... | codellama |
| 创意 | story, poem, fiction, narrative... | mistral |
| 分析 | analyze, compare, evaluate, research... | llama3 |
| 数学 | calculate, equation, solve, proof... | deepseek-math |
| 翻译 | translate, language, convert... | llama3 |
| 通用 | (默认) | llama3 |
